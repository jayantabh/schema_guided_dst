...
FILE_RANGES = {
    "dstc8_single_domain": {
        "train": range(1, 44),
        "dev": range(1, 2), # Changing temp to 1, 2
        "test": range(1, 12)
    },
    "dstc8_multi_domain": {
        "train": range(44, 128),
...
class Dstc8DataProcessor(object):
    ...
    def _create_examples_from_turn(self, turn_id, system_utterance,
        ...
        additional_utterance = "sys: " + system_utterance + " user:" + user_utterance
        additional_tokens, additional_alignments, additional_inv_alignments = (
            self._tokenize(additional_utterance))
        states = {}
        ...
        for service, user_frame in user_frames.items():
            ...
            example.add_additional_features(additional_tokens, additional_inv_alignments)
...
class InputExample(object):
    ...
    def __init__(self,
        ...
        self.intent_utt_ids = [0] * self._max_seq_length
        self.intent_utt_segment = [0] * self._max_seq_length
        self.intent_utt_mask = [0] * self._max_seq_length
        self.intent_utt_start_char_idx = [0] * self._max_seq_length
        self.intent_utt_end_char_idx = [0] * self._max_seq_length

        self.cat_slot_utt = []
        self.cat_slot_utt_seg = []
        self.cat_slot_utt_mask = []
        self.cat_slot_utt_start_char_idx = []
        self.cat_slot_utt_end_char_idx = []

        self.non_cat_slot_utt = []
        self.non_cat_slot_utt_seg = []
        self.non_cat_slot_utt_mask = []
        self.non_cat_slot_utt_start_char_idx = []
        self.non_cat_slot_utt_end_char_idx = []
    ...
    def add_utterance_features(self, system_tokens, system_inv_alignments,
        ...
        Args:
          ...
          system_inv_alignments: a list of tuples which denotes the start and end
            character of the token that a bert token originates from in the original
            system utterance.
    ...
    def add_additional_features(self, additional_tokens, additional_inv_alignments):
        """Add utterance related features input to bert.
        ...
        Args:
          additional_tokens: a list of strings which represents system utterance.
          additional_inv_alignments: a list of tuples which denotes the start and end
            character of the token that a bert token originates from in the original
            system utterance.
        """
        # Make user-system utterance input (in BERT format)
        ...
        base_utt_subword.append("[CLS]")
        base_utt_seg.append(0)
        base_utt_mask.append(1)
        base_start_char_idx.append(0)
        base_end_char_idx.append(0)

        #TODO: Change inference code too

        for subword_idx, subword in enumerate(additional_tokens):
            base_utt_subword.append(subword)
            base_utt_seg.append(0)
            base_utt_mask.append(1)
            st, en = additional_inv_alignments[subword_idx]
            base_start_char_idx.append(st + 1)
            base_end_char_idx.append(en + 1)

        intent_utt_subword = base_utt_subword[:]
        intent_utt_seg = base_utt_seg[:]
        intent_utt_mask = base_utt_mask[:]
        intent_start_char_idx = base_start_char_idx[:]
        intent_end_char_idx = base_end_char_idx[:]

        intent_utt_subword.append("[SEP]")
        intent_utt_seg.append(0)
        ...
        for subword_idx, subword in enumerate(intent_desc_tokens):
            ...
            #TODO: Check this if doesn't work
        ...
        intent_utterance_ids = self._tokenizer.convert_tokens_to_ids(intent_utt_subword)

        # Zero-pad up to the BERT input sequence length.
        while len(intent_utterance_ids) < max_utt_len:
            intent_utterance_ids.append(0)
            intent_utt_seg.append(0)
            intent_utt_mask.append(0)
            intent_start_char_idx.append(0)
            intent_end_char_idx.append(0)

        self.intent_utt_ids = intent_utterance_ids
        self.intent_utt_segment = intent_utt_seg
        self.intent_utt_mask = intent_utt_mask
        self.intent_utt_start_char_idx = intent_start_char_idx
        self.intent_utt_end_char_idx = intent_end_char_idx

        categorical_slots = self.service_schema.categorical_slots

        for slot_idx, slot in enumerate(categorical_slots):
            cat_slot_utt_subword = base_utt_subword[:]
            cat_slot_utt_seg = base_utt_seg[:]
            cat_slot_utt_mask = base_utt_mask[:]
            cat_slot_start_char_idx = base_start_char_idx[:]
            cat_slot_end_char_idx = base_end_char_idx[:]

            cat_slot_utt_subword.append("[SEP]")
            ...
            cat_slot_utt_subword.append("null")
            ...
            cat_slot_utterance_ids = self._tokenizer.convert_tokens_to_ids(cat_slot_utt_subword)

            # Zero-pad up to the BERT input sequence length.
            while len(cat_slot_utterance_ids) < max_utt_len:
                cat_slot_utterance_ids.append(0)
                cat_slot_utt_seg.append(0)
                cat_slot_utt_mask.append(0)
                cat_slot_start_char_idx.append(0)
                cat_slot_end_char_idx.append(0)
        ...
        for slot_idx, slot in enumerate(non_categorical_slots):
            non_cat_slot_utt_subword = base_utt_subword[:]
            non_cat_slot_utt_seg = base_utt_seg[:]
            non_cat_slot_utt_mask = base_utt_mask[:]
            non_cat_slot_start_char_idx = base_start_char_idx[:]
            non_cat_slot_end_char_idx = base_end_char_idx[:]

            non_cat_slot_utt_subword.append("null")
            non_cat_slot_utt_seg.append(0)
            non_cat_slot_utt_mask.append(1)
            non_cat_slot_start_char_idx.append(0)
            non_cat_slot_end_char_idx.append(0)

            non_cat_slot_utt_subword.append("[SEP]")
            non_cat_slot_utt_seg.append(0)
            non_cat_slot_utt_mask.append(1)
            non_cat_slot_start_char_idx.append(0)
            non_cat_slot_end_char_idx.append(0)
            ...
            non_cat_slot_utterance_ids = self._tokenizer.convert_tokens_to_ids(non_cat_slot_utt_subword)

            # Zero-pad up to the BERT input sequence length.
            while len(non_cat_slot_utterance_ids) < max_utt_len:
                non_cat_slot_utterance_ids.append(0)
                non_cat_slot_utt_seg.append(0)
                non_cat_slot_utt_mask.append(0)
                non_cat_slot_start_char_idx.append(0)
                non_cat_slot_end_char_idx.append(0)
...
def file_based_convert_examples_to_features(dial_examples, output_file):
    ...
    for (ex_index, example) in enumerate(dial_examples):
        ...
        features["cat_utt_mask"] = _create_int_feature(ex.cat_slot_utt_mask)

        features["non_cat_utt"] = _create_int_feature(ex.non_cat_slot_utt)
        features["non_cat_utt_seg"] = _create_int_feature(ex.non_cat_slot_utt_seg)
        features["non_cat_utt_mask"] = _create_int_feature(ex.non_cat_slot_utt_mask)
