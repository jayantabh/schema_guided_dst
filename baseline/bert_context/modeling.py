...
def embedding_postprocessor(input_tensor,
                            ...
                            context_vocab_size=2,
                            context_type=None,
                            context_embedding_name_prefix="context_embeddings",
                            initializer_range=0.02,
                            max_position_embeddings=512,
                            dropout_prob=0.1):
    ...
    Args:
      ...
      context_ids: (optional) context_ids.
      context_vocab_size: context_vocab_size.
      context_type: context_type.
      context_embedding_name_prefix: context_embedding_name_prefix
    ...
    if context_type is not None:
        if context_ids is None:
            raise ValueError("`context_ids` must be specified if"
                             "`use_context_embeddings` is True.")
        context_embed_table = tf.get_variable(
            name=context_embedding_name_prefix + context_type,
            shape=[context_vocab_size, width],
            initializer=create_initializer(initializer_range))
        # This vocab will be small so we always do one-hot here, since it is always
        # faster for a small vocabulary.
        flat_context_type_ids = tf.reshape(context_ids, [-1])
        one_hot_ids = tf.one_hot(flat_context_type_ids, depth=context_vocab_size)
        context_embeddings = tf.matmul(one_hot_ids, context_embed_table)
